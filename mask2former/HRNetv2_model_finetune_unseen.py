...
class HRNet_W48_Finetune_Unseen_ARCH(nn.Module):
    """
    ...
    def __init__(self, *, 
        ...
        super(HRNet_W48_Finetune_Unseen_ARCH, self).__init__()
        ...
        self.unseen_remap_dict = unseen_remap_dict
        self.useful_link_num = 0
        self.remap_list = self.ignore_lb * torch.ones(self.num_unify_classes, dtype=torch.long)
        self.id_map = torch.zeros(self.num_unify_classes)
        cur_k = 0

        for k,v in self.unseen_remap_dict.items():
            self.remap_list[k] = v
            if v != self.ignore_lb:
                self.useful_link_num += 1
                self.id_map[cur_k] = v
                cur_k += 1
    ...
    def from_config(cls, cfg):
        ...
        unseen_remap_dict = {}
        with open('output/camvid_map.txt', 'r') as file:
            lines = file.readlines()
            for line in lines:
                # print(line)
                k,v = line.split(':')
                unseen_remap_dict[int(k)] = int(v)
        ...
        return {
            ...
            "unseen_remap_dict": unseen_remap_dict
        }
    ...
    def forward(self, batched_inputs, dataset=0):
        ...
        if self.training:
            raise Exception("Only use for evaluate unseen datasets")
        else:
            processed_results = []
            for logit, input_per_image, image_size, uni_logits in zip(outputs['logits'], batched_inputs, images.image_sizes, outputs['uni_logits']):
                ...
                temp = uni_logits[:, self.remap_list[self.remap_list != self.ignore_lb], :, :]
                logger.info(self.remap_list[self.remap_list != self.ignore_lb])
                uni_logits = F.interpolate(temp, size=(images.tensor.shape[2], images.tensor.shape[3]), mode="bilinear", align_corners=True)
                ...
                this_mseg_map = self.id_map.cuda()
